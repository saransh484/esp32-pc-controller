...
A lightweight, automated system for remote infrastructure management using an ESP32. This project implements a pull-based HTTPS firmware update architecture alongside an MQTT-driven host management interface. Built with a zero-secrets build pipeline to ensure open-source compliance.

## System Architecture

- **Hardware Interface:** ESP32-CAM platform utilizing GPIO 4 (Status indicator toggle), GPIO 12 (Host Power), and GPIO 13 (Host Reset).
- **Firmware Management:** Client-driven HTTP polling loop utilizing the Arduino `HTTPUpdate` and `ArduinoJson` libraries. Updates are validated via metadata payloads and pulled down via Nginx/Caddy proxies over TLS.
